...
const { data: existingProfile } = await supabase
    ...
    .eq("user_id", user.id)
    .maybeSingle();
update.public_id = existingProfile?.public_id || "user_" + randomUUID().split("-")[0].slice(0, 8);
update.inbox_email_alias = existingProfile?.inbox_email_alias || randomUUID().split("-")[0];

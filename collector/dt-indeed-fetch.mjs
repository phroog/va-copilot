...
const EXPR = `(async () => {
  ...
  const beacon = (t.match(/job_seen_beacon/g)||[]).length;
  const jk = (t.match(/data-jk/g)||[]).length;
  const jcs = (t.match(/jcs-JobTitle/g)||[]).length;
  return JSON.stringify({ status: r.status, bytes: t.length, beacon, jk, jcs, challenge: /captcha|unusual traffic/i.test(t.slice(0,3000)) });
})()`;
